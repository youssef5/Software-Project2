//Author Michelle Smith/Youssef Beshay
#include<stdio.h>
#include<string.h>
#include<stdlib.h>
#include<time.h>
...
void printDetails(player playerArray[],int noOfPlayers)
{
    ...
    for(int i=0;i<noOfPlayers;i++)
    {
        ...
        printf("%s %s magicSkills %d smartness %d luck %d dexterity %d strength %d lifePoints %d\n\n",playerArray[i].name,playerArray[i].type,playerArray[i].magicSkills,playerArray[i].smartness,playerArray[i].luck,playerArray[i].dexterity,playerArray[i].strength,playerArray[i].lifePoints);
    }
}
...
void updateCapabiltitesOnMovingSlot(player playerArray[],int playerNumber,int slotTypeMovedTo)
{
    //if player moved to a hill
    if(slotTypeMovedTo==1)
    {
        //checks if the player dexerity less than 50
        if(playerArray[playerNumber].dexterity<50)
        {
            //updates the strength of the player
            playerArray[playerNumber].strength-=10;
        }
        //checks if the player dexerity is greater than 60
        else if(playerArray[playerNumber].dexterity>=60)
        {
            //updates the player strength
            playerArray[playerNumber].strength+=10;
        }
    }
    ...
    {
        //checks if the player smartness is greater than 60
        if(playerArray[playerNumber].smartness>60)
        {
            //updates the player magic skills
            playerArray[playerNumber].magicSkills+=10;
        }
        //checks if the player smartness is less than 50
...
void implementingAttack(player playerArray[],int attackerPlayerNo,int attackedPlayerNo)
{
    //checks if the attacked player strength is less than 70
    if(playerArray[attackedPlayerNo].strength<=70)
    {
        //updates the lifePoints of the attacked player
    ...
    {
        //updates the lifePoints of the attacker player
